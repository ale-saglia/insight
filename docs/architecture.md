...
│   └── ensure-frontmatter.sh   # Validates and normalises frontmatter for all articles
...
└── Makefile                    # Shortcuts: setup / build / serve / rebuild / clean
```
...
`make build` runs `scripts/build-local.sh`, which:
