...
RELATIVE_URLS = False
DELETE_OUTPUT_DIRECTORY = False  # _site/ managed by CI cache step
```
...
- **Python**: authoritative source is `.python-version`. CI (`pages.yml`) reads from it via `python-version-file`. `check-versions.yml` verifies that `.devcontainer/Dockerfile` stays in sync.
...
make check-links    # linkchecker offline link check on _site/ (requires: make setup-dev)
...
3. `pip install uv`
4. `apt-get install fonts-dejavu-core` (system fonts required by OG image generation)
5. `uv pip install --require-hashes -r requirements-dev.txt --system`
6. `python scripts/ensure_frontmatter.py`
7. `pelican --settings publishconf.py` (OG images generated by plugin during this step)
8. `linkchecker` offline link check on `_site/`
9. Upload `_site/` as GitHub Pages artifact
