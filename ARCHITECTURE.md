...
Insight Notes is a static site built with [Pelican 4.10.2](https://getpelican.com) (Python). The published output is a set of plain HTML files deployed to GitHub Pages.
...
## The `insight` plugin (`plugins/insight.py`)

All site-specific logic lives in a single plugin. It hooks into three Pelican signals:
...
### `process_articles` / `_enrich_article`
...
make build          # ensure-frontmatter.sh → generate-og-images.sh → pelican
make serve          # pelican build (no OG regen) → python3 -m http.server 4000
make rebuild        # rm -rf _site && make build
```

VS Code tasks mirror these under "Pelican: *".

OG images are generated by `scripts/og-image-gen/generate.js` (Node.js + Puppeteer). The script reads `SITENAME`, `SITESUBTITLE`, etc. from `pelicanconf.py` via regex.

### CI (GitHub Actions)

`.github/workflows/pages.yml`:
1. `setup-python` using `PYTHON_VERSION` from `versions.env`
2. `pip install -r requirements.txt`
3. `pelican --settings publishconf.py`
