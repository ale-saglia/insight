...
The model uses BPMN 2.0, with two swim lanes representing the two functional roles involved: *Pattuglia su strada* (the patrol unit handling on-site detection and immediate contestation) and *Ufficio Verbali* (the administrative unit handling documentation, notification, and all subsequent phases).

![Processo sanzionatorio al Codice della Strada — BPMN 2.0](https://raw.githubusercontent.com/ale-saglia/insight/refs/heads/main/assets/process-sanctions-cds-en.bpmn.svg)
...
The event-based gateway after notification handles three mutually exclusive waiting states: payment within 60 days (timer event), a request for defensive writings (message event), and simple inaction (which triggers the 60-day timer). This was the hardest part of the model to get right, because in practice the three outcomes feel sequential rather than parallel. The gateway makes explicit that the process is actually suspended, waiting for one of three things to happen.
